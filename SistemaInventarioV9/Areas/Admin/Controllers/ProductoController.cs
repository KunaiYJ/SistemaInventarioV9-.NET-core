...
namespace SistemaInventarioV9.Areas.Admin.Controllers
{
    [Area("Admin")]
    public class ProductoController : Controller
    {
        ...
        public async Task<IActionResult> Upsert(ProductoVM productoVM)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    // ACTUALIZAR
                    var objProducto = await _unidadTrabajo.Producto.ObtnerPrimero(p => p.Id == productoVM.Producto.Id, isTracking: false);
                    if (files.Count > 0) //CARGAMOS UNA NUEVA IMAGEN
                    {
                        string upload = webRootPath + DS.ImagenRuta;
                        string fileName = Guid.NewGuid().ToString();
                        string extension = Path.GetExtension(files[0].FileName);
                        ...
                        productoVM.Producto.ImagenUrl = fileName + extension;
                    } // SI NO SE CARGA UNA NUEVA IMAGEN
                    else
                    {
